...
async function configure(h) {
  const page=h.page;
  await page.locator('#normalization-settings').click();
  assert.equal(await page.locator('[data-project]:checked').count(),0,'do not automatically put the atlas into one batch');
  for (const el of await page.locator('[data-project]').all()) await el.check();
  ...
  await page.locator('#normalization-form tbody tr').filter({hasText:'master-a'}).locator('input[type=checkbox]').check();
}
  ...
  try {
    ...
    await page.evaluate(async()=>{
      ...
      for(const p of await ProjectStorage.listProjects()) {
        ...
        await ProjectStorage.putProject(p);
        window.__cloudRows[p.id]={id:p.id,display_name:p.displayName,folder_path:[],meta:Cloud.metaOf(p),state:Cloud.stateOf(p),updated_at:p.cloudUpdatedAt,bundle_path:p.cloudBundlePath};
      }
