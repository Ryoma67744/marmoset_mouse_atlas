...
(function (global) {
  ...
  const REASONS = {
    ...
    INTERNAL_STANDARD_MISSING: '内部標準がないため補正をスキップしました。未補正の生値を表示します。',
  ...
  function referenceValid(reference, allowEmpty) {
    const text = value => typeof value === 'string' && value.trim().length > 0;
    return !!reference && typeof reference === 'object' && !Array.isArray(reference) && ['qc', 'roi', 'whole_tissue'].includes(reference.kind) &&
      Array.isArray(reference.projectIds) && (allowEmpty === true || reference.projectIds.length > 0) && reference.projectIds.every(text) &&
  ...
  function isSkippedProfile(profile) {
    return !!profile && profile.schemaVersion === SIMPLE_SCHEMA_VERSION && !!profile.application &&
      profile.application.status === 'skipped' && profile.application.reasonCode === 'INTERNAL_STANDARD_MISSING';
  }
  ...
  function suggestSimpleMapping(project, rasters) {
    ...
    // A declared standard with an unreadable numeric source is not absent.
    // Keep all declarations in ambiguity checks so a failed load cannot choose
    // another candidate or silently convert an error into a skipped section.
    const d4 = candidates((project || {}).molecules, 'd4').map(m => m.key), ht = candidates(molecules, 'ht').map(m => m.key);
    const issues = [];
    const standardKey = d4.length === 1 && molecules.some(m => m.key === d4[0]) ? d4[0] : null;
    if (d4.length > 1) issues.push('D4_AMBIGUOUS');
    else if (d4.length && !standardKey) issues.push('RAW_MISSING');
    if (ht.length > 1 && d4.length) issues.push('HT_AMBIGUOUS');
    ...
    return { standardKey: standardKey, htKey: ht.length === 1 ? ht[0] : null, targetKeys: targetKeys,
      ...
      skipEligible: d4.length === 0, issues: issues, candidates: { d4: d4, ht: ht } };
  }
  function simpleReferenceValid(reference, allowEmpty) {
    if (!reference) return false;
    return referenceValid(Object.assign({}, reference, { kind: reference.kind === 'd4_measured' ? 'qc' : reference.kind }), allowEmpty) &&
      (reference.kind !== 'roi' || reference.roiNames && reference.roiNames.length > 0 || allowEmpty === true && reference.projectIds.length === 0) &&
  ...
  function simpleFingerprint(profile) {
    ...
    if (Object.prototype.hasOwnProperty.call(profile, 'application')) value.application = profile.application;
    return digest(value);
  }
  function simpleMapping(entry) {
    ...
    if (!standardKey && suggestion.skipEligible) return { skipped: true,
      mapping: { ht: null, d4: null, da: null, ne: null }, targetKeys: [], excludedKeys: (project.molecules || []).map(m => m.key) };
    if (!standardKey) {
      const code = suggestion.candidates.d4.length > 1 ? 'D4_AMBIGUOUS' : suggestion.issues.includes('RAW_MISSING') ? 'RAW_MISSING' : 'D4_MISSING';
      const error = new Error(reasonText(code)); error.code = code; error.projectId = project.id; throw error;
    }
  ...
  function simpleTargetsValid(profile, project) {
    ...
    if (isSkippedProfile(profile)) return Array.isArray(targets) && targets.length === 0 && ROLES.every(role => mapping[role] === null) &&
      candidates(project.molecules, 'd4').length === 0 && !!profile.section && ['Ds', 'Dref', 'k'].every(key => profile.section[key] === null);
  ...
  function createSimpleProfiles(entries, config) {
    ...
    // Empty references are provisional here and accepted below only when every
    // section has a genuinely absent standard and will be recorded as skipped.
    if (!simpleReferenceValid(config.reference, true)) throw new Error(reasonText('REFERENCE_INVALID'));
    ...
    if (refIds.some(id => !localIds.includes(id))) throw new Error(reasonText('REFERENCE_INVALID'));
    config.scope.memberIds.sort();
    ...
    const calculated = entries.map(entry => {
      ...
      if (resolved.skipped && numericMolecules(entry.project, entry.rasters).some(m =>
        !moleculeCoordinatesValid(entry.project, m.key, rasterOf(entry.rasters, m.key, entry.project)))) {
        const error = new Error((entry.project.displayName || entry.project.name || entry.project.id) + ': ' + reasonText('COORDINATE_MISMATCH'));
        error.code = 'COORDINATE_MISMATCH'; error.projectId = entry.project.id; throw error;
      }
      return Object.assign({ entry: entry, rawFingerprint: fingerprint(entry.project, entry.rasters),
        geometryFingerprint: geometryFingerprint(entry.project, config.reference),
        section: resolved.skipped ? { Ds: null, Dref: null, k: null, nGeometry: 0, nMeasured: 0, nValid: 0, coverage: null,
          reasonCounts: {}, reasonCodes: ['INTERNAL_STANDARD_MISSING'], status: 'SKIPPED' } :
          simpleSectionStats(entry.project, entry.rasters, resolved.mapping, config) }, resolved);
    });
    const active = calculated.filter(c => !c.skipped);
    if (active.length && !simpleReferenceValid(config.reference)) throw new Error(reasonText('REFERENCE_INVALID'));
    if (config.mode === 'simple' && active.some(c => !refIds.includes(c.entry.project.id))) throw new Error(reasonText('REFERENCE_INVALID'));
    ...
    const refs = active.filter(c => refIds.includes(c.entry.project.id));
    if (active.length && !refs.length) throw new Error(reasonText('REFERENCE_INVALID'));
    const allReferencesValid = refs.length > 0 && refs.every(c => Number.isFinite(c.section.Ds) && c.section.Ds > 0);
    const Dref = allReferencesValid ? median(refs.map(c => c.section.Ds)) : null;
    ...
    const profiles = calculated.map(c => {
      ...
      if (c.skipped) profile.application = { status: 'skipped', reasonCode: 'INTERNAL_STANDARD_MISSING' };
    ...
    const preview = profiles.map((item, i) => {
      ...
      return Object.assign({ projectId: item.projectId, name: entries[i].project.displayName || entries[i].project.name || '', profileId: item.normalization.id,
        revision: item.normalization.revision, quality: item.normalization.quality, skipped: isSkippedProfile(item.normalization), correctedOutputs: correctedOutputs,
        totalFiniteOutput: correctedOutputs.reduce((sum, output) => sum + output.nValid, 0) }, clone(item.normalization.section));
    });
    const saveReasons = [];
    if (preview.some(p => !p.skipped && !(Number.isFinite(p.k) && p.k > 0))) saveReasons.push('REFERENCE_PARTIAL');
    if (active.length && !preview.some(p => p.totalFiniteOutput > 0)) saveReasons.push('SIMPLE_NO_TARGETS');
    ...
    profiles.forEach(item => {
      ...
      if (isSkippedProfile(profile) || !profile.calibration) return;
    ...
    return { profiles: profiles, preview: preview, canSave: !saveReasons.length, reasonCodes: unique(saveReasons),
      summary: { total: profiles.length, corrected: active.length, skipped: profiles.length - active.length } };
  }
  function evaluateSimple(project, rasters) {
    ...
    const mapping = profile.mapping || {}, referenceValidNow = simpleReferenceValid(profile.reference, skipped), validScope = scopeValid(profile.scope);
    ...
    for (const m of project.molecules || []) {
      const target = skipped ? null : (Array.isArray(profile.targets) ? profile.targets : []).find(t => t && t.key === m.key);
      const role = skipped ? 'other' : m.key === mapping.d4 ? 'd4' : target ? ROLES.find(r => mapping[r] === m.key) || 'analyte' : 'other';
      ...
      if (skipped) {
        channel.status = reasons.length ? 'UNAVAILABLE' : 'SKIPPED';
        channel.reasonCodes = unique(['INTERNAL_STANDARD_MISSING'].concat(reasons));
        continue;
      }
    ...
    if (skipped) return { profile: profile, status: reasons.length ? 'UNAVAILABLE' : 'SKIPPED',
      reasonCodes: unique(['INTERNAL_STANDARD_MISSING'].concat(reasons)), channels: channels, section: profile.section || null, fingerprint: currentFingerprint };
  ...
  function quantifyRoi(project, rasters, evaluation, mask) {
    ...
    return (project.molecules || []).map(m => {
      ...
      const applicable = isCorrectedChannel(channel), simple = profile && profile.schemaVersion === SIMPLE_SCHEMA_VERSION, skipped = isSkippedProfile(profile);
      ...
      } else if (skipped) reasons.push.apply(reasons, channel ? channel.reasonCodes : ['INTERNAL_STANDARD_MISSING']);
      ...
      if (skipped) { absolute.status = 'NOT_APPLICABLE'; absolute.reasonCodes = ['INTERNAL_STANDARD_MISSING']; }
      else if (role !== 'ht') { absolute.status = 'NOT_APPLICABLE'; absolute.reasonCodes = ['ABSOLUTE_NOT_APPLICABLE']; }
      ...
      const status = skipped ? channel && channel.status === 'SKIPPED' ? 'SKIPPED' : 'UNAVAILABLE' : !applicable ? role === 'd4' ? 'RAW_QC' : 'NOT_APPLIED' :
  ...
  global.Normalization = { SCHEMA_VERSION: SCHEMA_VERSION, LEGACY_SCHEMA_VERSION: LEGACY_SCHEMA_VERSION, METHOD_VERSION: METHOD_VERSION, suggestMapping: suggestMapping,
    ...
    createSimpleProfiles: createSimpleProfiles, suggestSimpleMapping: suggestSimpleMapping, isInternalStandard: isInternalStandard, isSkippedProfile: isSkippedProfile,
