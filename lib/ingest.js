...
(function (global) {
  ...
  // Missing cells must be identified before Number(): Number(null), Number('')
  // and Number('   ') are zero. Keep true numerical zero as measured signal.
  function numericCell(value) {
    if (value == null || typeof value === 'boolean') return NaN;
    if (typeof value === 'string') {
      value = value.trim();
      if (!value || /^(?:NA|N\/A|NaN|NULL)$/i.test(value)) return NaN;
    } else if (typeof value !== 'number') return NaN;
    const number = Number(value);
    return Number.isFinite(number) ? number : NaN;
  }

  // Preserve empty delimited fields and quoted cells, including quoted commas.
  function splitTextRow(line, separator) {
    if (separator !== ',' && separator !== '\t') return line.trim().split(separator);
    ...
    for (let i = 0; i < line.length; i++) {
      ...
      } else if (char === separator && !quoted) { cells.push(cell); cell = ''; }
      else cell += char;
    }
    cells.push(cell);
    return cells;
  ...
  function rowsFromText(raw, def) {
    ...
    if (isAnalyte) {
      ...
      for (let i = 4; i < lines.length; i++) {
        const tok = lines[i].trim().split(/\s+/).map(numericCell);
        if (tok.length >= 3 && Number.isFinite(tok[1]) && Number.isFinite(tok[2])) {
          rows.push({ x: tok[1], y: tok[2], v: numericCell(tok[valueColIndex]) });
        }
      }
    ...
    const head = splitTextRow(lines[0], sep).map(s => s.trim());
    ...
    for (let i = 1; i < lines.length; i++) {
      const tok = splitTextRow(lines[i], sep);
      const x = numericCell(tok[xi]), y = numericCell(tok[yi]), v = numericCell(tok[vi]);
      // Missing intensity retains its coordinate so an empty edge does not
      // shift or shrink the raster relative to the other molecular channels.
      if (Number.isFinite(x) && Number.isFinite(y)) rows.push({ x, y, v });
    }
  ...
  function a1ColToIndex(colRef) {
    if (typeof colRef === 'number') return Number.isFinite(colRef) ? colRef : NaN;
    // ★ null / undefined を String() に通すと "NULL" / "UNDEFINED" が
    //   そのまま A-Z として解釈され、巨大な列番号になってしまう。
    //   呼び出し側の「列指定が不正」チェックが死ぬので、ここで弾く。
    if (typeof colRef !== 'string' || !/^[A-Za-z]{1,3}$/.test(colRef)) return NaN;
    const s = colRef.toUpperCase();
    let n = 0;
  ...
  function rowsFromWorkbook(wb, def) {
    ...
    for (let i = startIdx; i < aoa.length; i++) {
      const r = aoa[i];
      if (!r) continue;
      const x = numericCell(r[xi]), y = numericCell(r[yi]), v = numericCell(r[vi]);
      if (Number.isFinite(x) && Number.isFinite(y)) rows.push({ x, y, v });
    }
    if (!rows.length) throw new Error(`xlsx に有効な座標行がありません (sheet=${sheetName})`);
    return rows;
  }
  ...
  global.Ingest = {
    ...
    decodeTiffArrayBuffer, imageBlobToDataUrl, isTiffName,
  };
})(window);
