...
(function (global) {
  ...
  function renderSection(section, options) {
    ...
    if (opts.roiRegion) {
      const mask = new Uint8Array(section.W * section.H);
      for (const polygon of roiPolygons(section).filter(p => p.visible && p.name === opts.roiRegion))
        global.MSIRaster.markRoiMask(mask, section.W, section.H, polygon.vertices);
      for (const canvas of [result.canvas, result.previewCanvas]) {
        const ctx = canvas.getContext('2d'), image = ctx.getImageData(0, 0, section.W, section.H);
        for (let i = 0; i < mask.length; i++) if (!mask[i]) image.data.fill(0, i * 4, i * 4 + 4);
        ctx.putImageData(image, 0, 0);
      }
      const pixels = result.canvas.getContext('2d').getImageData(0, 0, section.W, section.H).data;
      result.visiblePixels = mask.reduce((n, _, i) => n + (pixels[i * 4 + 3] > 0 ? 1 : 0), 0);
    }
  ...
  function roiPolygons(section) {
    ...
    for (const [key, polygons] of Object.entries(roi.roi_items || {})) {
      ...
      for (const polygon of polygons) {
        ...
        records.push({ key, name: String(roi.roi_names?.[key] || key).normalize('NFC').trim(), vertices, color, visible: !(roi.roi_show_flags && roi.roi_show_flags[key] === false) });
      }
    }
    return records;
  }
  ...
  function paintRoiPreview(section, canvas, options) {
    ...
    ctx.save();
    // Points below are already projected to final canvas pixels. The MSI
    // preview context still carries its image orientation transform.
  ...
  global.Stack3D = { ANALYTES, COLORS: global.SectionDisplay.COLORS, naturalCor, sortCoronal, channelMap, geometry,
    createSection, loadSections, computeCommonRanges, renderSection, orientCanvas, roiPolygons, roiSummary, loadReferenceImage, renderHePlane, renderHePreview, releaseSection };
})(window);
