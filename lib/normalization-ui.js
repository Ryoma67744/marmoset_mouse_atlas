...
(function (global) {
  ...
  function open(options) {
    ...
    let busy = false, entries = [], preview = null, saved = null, cloudPending = new Set(), savedKind = 'normalization';
    ...
    function displayModule() {
      if (!global.DisplayRange) throw new Error('表示レンジ機能を読み込めませんでした。ページを再読み込みしてください。');
      return global.DisplayRange;
    }
    function displayEntries(updates, rastersById) {
      return updates.map(item => ({project:item.project,rasters:rastersById.get(item.project.id)}));
    }
    function cloneDisplaySettings(project) {
      // prepareProject and range setters intentionally mutate display records.
      // Keep every reviewed storage object untouched until the CAS commits.
      project.valueDisplay = JSON.parse(JSON.stringify(project.valueDisplay || {}));
      project.layerDisplay = JSON.parse(JSON.stringify(project.layerDisplay || {}));
      return project;
    }
    function initializeDisplaySettings(updates, rastersById) {
      const D = displayModule();
      for (const item of updates) {
        const p = cloneDisplaySettings(item.project);
        const evaluation = global.Normalization.evaluate(p,rastersById.get(p.id));
        D.prepareProject(p,evaluation);
        for (const molecule of p.molecules || []) D.setStrategy(p,molecule.key,'normalized','individual');
        p.valueDisplay.mode = global.Normalization.isSkippedProfile(p.normalization) ? 'raw' : 'normalized';
        p.valueDisplay.scale = 'individual';
      }
      // Legacy unscoped profiles keep individual display. Their numerical
      // snapshot must not be converted to folder scope by a display operation.
      if (scoped) {
        const snapshot = D.buildGroupSnapshot(displayEntries(updates,rastersById));
        for (const item of updates) item.project.valueDisplay.groupRangeSnapshot = snapshot;
      }
    }
    function sharedProfileSignature(profile) {
      return global.Normalization.stableStringify(Object.fromEntries([
      ...
      ].map(key=>[key,profile && profile[key]])));
    }
    function canRefreshDisplay() {
      if (!scoped || !activeGroup || !mergedGuard || !localGuard || activeGroup.reassignGroupId || activeGroup.identityConflict || !entries.length) return false;
      const group = Object.assign({},activeGroup,{projects:entries.map(e=>e.project)});
      const shared = sharedProfileSignature(entries[0].project.normalization);
      return entries.every(entry=>Scope.assess(entry.project,group).status === 'CURRENT' && sharedProfileSignature(entry.project.normalization) === shared);
    }
    function updateDisplayControls() {
      const button = body.querySelector('#normalization-display-refresh');
      if (button) button.disabled = !canRefreshDisplay();
      const retry = body.querySelector('#normalization-display-cloud');
      if (retry) retry.disabled = savedKind !== 'display' || !cloudPending.size || !options.saveCloud;
    }
    async function syncDisplaySaved() {
      if (!saved || savedKind !== 'display' || !options.saveCloud || !cloudPending.size) return;
      const failures = [];
      for (const p of saved.filter(p=>cloudPending.has(p.id))) {
        status.textContent = '共通表示レンジはこの PC に保存済み。クラウド同期中：' + (p.displayName || p.id);
        try { await options.saveCloud(p); cloudPending.delete(p.id); }
        catch (e) { failures.push((p.displayName || p.id) + '：' + (e.message || String(e))); }
      }
      status.textContent = '共通表示レンジを ' + saved.length + ' 件で更新しました。補正値・補正設定・ROI・Otsu は保持しています。クラウド同期 ' +
        (saved.length-cloudPending.size) + '/' + saved.length + ' 件。' + (failures.length ? '\n未完了分は再試行できます。\n' + failures.join('\n') : '');
      status.style.color = failures.length ? '#92400e' : '#1e293b';
    }
    function mountDisplayControls() {
      if (!scoped) return;
      const controls = document.createElement('div');
      controls.style.cssText = 'border-top:1px solid #cbd5e1;margin-top:16px;padding-top:12px';
      controls.innerHTML = '<button class="btn" type="button" id="normalization-display-refresh">共通表示レンジを更新</button> ' +
        '<button class="btn" type="button" id="normalization-display-cloud" disabled>表示レンジのクラウド未完了分を再試行</button>' +
        '<p style="font-size:12px">保存済みの補正値から、同じフォルダー・同じ分子の表示上限（P99）を更新します。フォーム内の未保存の補正条件は使用しません。グループ全件に同じ補正設定が保存されている場合に利用できます。</p>';
      body.appendChild(controls);
      updateDisplayControls();
      controls.querySelector('#normalization-display-refresh').onclick = async () => {
        if (busy || !canRefreshDisplay()) return;
        setBusy(true,'保存済み補正とフォルダー全件を確認して、共通表示レンジを計算中…');
        try {
          const D = displayModule(), N = global.Normalization;
          await checkScope(mergedGuard);
          if (!Scope.equalsSnapshot(localGuard,await readLocalGuard())) throw new Error('読み込み後にフォルダー構成が変更されました。設定を開き直してください。');
          const updates = [], verified = [];
          for (const entry of entries) {
            const latest = await options.storage.getProject(entry.project.id);
            if (!latest || latest.updatedAt !== entry.project.updatedAt || N.stableStringify(latest.normalization) !== N.stableStringify(entry.project.normalization)) throw new Error('読み込み後に更新されたデータがあります。設定を開き直してください。');
            const rasters = await N.loadRasters(latest,{storage:options.storage});
            if (N.fingerprint(latest,rasters) !== latest.normalization.rawFingerprint) throw new Error('生値が変更されています。補正設定を確認してください。');
            const evaluation = N.evaluate(latest,rasters);
            const invalid = (evaluation.reasonCodes || []).filter(code=>['NORMALIZATION_PROFILE_STALE','NORMALIZATION_SCHEMA_UNSUPPORTED','NORMALIZATION_SCOPE_INVALID','GROUP_MEMBERSHIP_CHANGED','REFERENCE_INVALID','MAPPING_INVALID','QC_THRESHOLDS_MISSING','COORDINATE_MISMATCH','RAW_MISSING'].includes(code));
            if (invalid.length) throw new Error((latest.displayName || latest.id) + '：保存済み補正を検証できません。' + invalid.map(N.reasonText).join(' / '));
            const project = cloneDisplaySettings(Object.assign({},latest));
            D.prepareProject(project,evaluation);
            verified.push({project,rasters,evaluation});
            updates.push({project,expectedUpdatedAt:latest.updatedAt});
          }
          const group = Object.assign({},activeGroup,{projects:verified.map(e=>e.project)});
          if (verified.some(e=>Scope.assess(e.project,group).status !== 'CURRENT')) throw new Error('保存時と補正グループの構成・識別情報が異なります。補正設定を確認してください。');
          const snapshot = D.buildGroupSnapshot(verified);
          updates.forEach(item=>{ item.project.valueDisplay.groupRangeSnapshot = snapshot; });
          await checkScope(mergedGuard);
          // The full membership guard also catches changes made while rasters
          // were being read. No group ID rewrite or numerical revision occurs.
          saved = await options.storage.putProjectsIfUnchanged(updates,localGuard);
          savedKind = 'display'; cloudPending = new Set(options.saveCloud ? saved.map(p=>p.id) : []);
          saved.forEach(p=>{ entries.find(e=>e.project.id === p.id).project = p; });
          preview = null;
          const normalSave = body.querySelector('#normalization-save');
          if (normalSave) normalSave.dataset.prevDisabled = '1';
          const normalRetry = body.querySelector('#normalization-cloud') || body.querySelector('#normalization-simple-cloud');
          if (normalRetry) normalRetry.dataset.prevDisabled = '1';
          status.textContent = '共通表示レンジを ' + saved.length + ' 件で更新しました。補正値・補正設定・ROI・Otsu は保持しています。';
          const warnings = [];
          if (options.onSaved) {
            try { await options.onSaved(saved); }
            catch (e) { warnings.push('保存は完了しています。画面の更新に失敗しました：' + (e.message || String(e))); }
          }
          await syncDisplaySaved();
          if (warnings.length) status.textContent += '\n' + warnings.join('\n');
          setBusy(false); updateDisplayControls();
        } catch (e) { setBusy(false); error(e); updateDisplayControls(); }
      };
      controls.querySelector('#normalization-display-cloud').onclick = async () => {
        if (busy || savedKind !== 'display' || !cloudPending.size) return;
        setBusy(true,'表示レンジのクラウド未完了分を再試行中…');
        try { await syncDisplaySaved(); }
        catch (e) { error(e); }
        finally { setBusy(false); updateDisplayControls(); }
      };
    }
    ...
    function renderDetailedConfiguration() {
      ...
      body.innerHTML = (scoped ? '<div data-configuration-folder="' + esc(activeGroup.folderId) + '" style="padding:10px;background:#eff6ff;margin:12px 0"><strong>' + esc(activeGroup.label) + '</strong> ／ 対象 ' + entries.length +
        '件（子フォルダーを含む全件）<p style="font-size:12px;margin-top:6px">D_ref はこのグループ内で計算します。補正後の画像は分子ごとの個別自動レンジで表示します。比較用の共通表示レンジ（P99）も保存します。DA・NE はこのフォルダー内の比較用です。</p>' +
      ...
      function invalidatePreview() {
        preview = null;
        if (savedKind !== 'display') { saved = null; cloudPending.clear(); }
      ...
      form.onsubmit = async e => {
        ...
        try {
          ...
          form.querySelector('#normalization-preview-result').innerHTML = '<p style="font-size:13px;margin:8px 0">対象 ' + entries.length + '件 ／ 係数算出可能 ' +
            ...
            '<p style="font-size:13px;margin:12px 0 4px">補正値の全範囲（参考・描画レンジではありません）' + (scoped ? '（' + esc(activeGroup.label) + '）' : '') + '</p>' +
      ...
      form.querySelector('#normalization-save').onclick = async () => {
        ...
        try {
          ...
          const updates = [], rastersById = new Map();
          for (const item of preview.profiles) {
            ...
            rastersById.set(latest.id,currentRasters);
          ...
          initializeDisplaySettings(updates,rastersById);
          ...
          if (scoped) {
            ...
            catch (e) { mergedGuard = null; localGuard = null; displayWarnings.push('対象一覧を再確認できませんでした。表示レンジを更新する場合は設定を開き直してください：' + (e.message || String(e))); }
          }
          setBusy(false, saved.length + ' 件をこの PC に保存しました。クラウドへの同期はまだ行っていません。');
          if (displayWarnings.length) status.textContent += '\n' + displayWarnings.join('\n');
          form.querySelector('#normalization-save').disabled = true;
      ...
      form.querySelector('#normalization-cloud').onclick = async () => {
        if (!saved || busy || !options.saveCloud || savedKind !== 'normalization') return;
    ...
    function renderSimpleConfiguration(switched) {
      ...
      // A previously skipped section may now have an uploaded standard. Reapply
      // must use the fresh candidates instead of retaining its empty mapping.
      const reuseMapping = entry => prior && !N.isSkippedProfile(entry.project.normalization);
      ...
      const selectedKeys = entries.map((entry, i) => reuseMapping(entry) ? (entry.project.normalization.targets || []).map(t => t.key) : suggestions[i].targetKeys);
      ...
      const hasAmbiguity = suggestions.some(s => !s.skipEligible && (s.issues || []).length);
      body.innerHTML = '<div data-configuration-folder="' + esc(activeGroup.folderId) + '" style="padding:10px;background:#eff6ff;margin:12px 0"><strong>' + esc(activeGroup.label) + '</strong> ／ 対象 ' + entries.length +
        ...
        '<form id="normalization-simple-form"><p id="normalization-simple-summary" style="font-size:13px;padding:8px;background:#f8fafc" role="status"></p>' +
        '<details id="normalization-mapping" ' + (hasAmbiguity && !prior ? 'open ' : '') + 'style="margin:12px 0"><summary>内部標準：d4-5-HT ／ 分子対応を確認</summary>' +
        '<table style="font-size:13px;width:100%;text-align:left"><thead><tr><th>データ</th><th>内部標準</th><th>画素比にする5-HT</th></tr></thead><tbody>' +
        entries.map((entry, i) => {
          const mapping = reuseMapping(entry) ? entry.project.normalization.mapping : {d4:suggestions[i].standardKey,ht:suggestions[i].htKey};
          const issues = reuseMapping(entry) ? [] : suggestions[i].issues || [];
          return '<tr data-simple-project="' + esc(entry.project.id) + '"><td style="padding:6px">' + esc(entry.project.displayName || entry.project.id) +
            '<small style="display:block">' + esc(projectPath(entry.project)) + '</small><small data-simple-application="' + i + '" style="display:block;color:#92400e"></small></td><td>' + mapSelect(entry, 'standard_' + i, mapping.d4, suggestions[i].skipEligible ? '内部標準なし（補正をスキップ）' : '内部標準を指定', issues.includes('D4_AMBIGUOUS')) +
            '</td><td>' + mapSelect(entry, 'ht_' + i, mapping.ht, '画素比の5-HTなし', issues.includes('HT_AMBIGUOUS') || (!reuseMapping(entry) && !mapping.ht && (suggestions[i].candidates.ht || []).length > 1)) + '</td></tr>';
        ...
        '<p style="font-size:12px">既定では d4 の実測領域の中央値を使い、内部標準がある切片を固定基準にします。内部標準がないデータは補正・基準計算をスキップします。実験条件の ID・版番号の手入力は不要です。</p>' +
        '<div style="display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:0 18px">' +
        ...
        '<p style="font-size:13px;font-weight:700">固定基準と補正対象</p>' + entries.map((entry, i) => '<fieldset style="margin:10px 0;padding:8px;border:1px solid #cbd5e1"><legend>' + esc(entry.project.displayName || entry.project.id) + '</legend>' +
          '<label><input type="checkbox" name="simple_ref_' + i + '" ' + (!prior || N.isSkippedProfile(entry.project.normalization) || (seed.reference.projectIds || []).includes(Scope.memberId(entry.project)) ? 'checked' : '') + '> 固定基準に含める</label><div style="margin-top:8px">' +
      ...
      const isSkippedEntry = i => !!suggestions[i].skipEligible && !form.elements['standard_' + i].value;
      const skipMessage = '内部標準なし・未補正（生値表示）';
      function updateSelections() {
        let total = 0, skippedCount = 0;
        entries.forEach((entry,i) => {
          const standard = form.elements['standard_' + i].value;
          const skipped = isSkippedEntry(i);
          if (skipped) skippedCount++;
          form.elements['ht_' + i].disabled = skipped;
          const reference = form.elements['simple_ref_' + i];
          if (skipped && !reference.disabled) {
            reference.dataset.beforeSkipChecked = reference.checked ? '1' : '0';
            reference.checked = false;
          } else if (!skipped && reference.disabled) {
            reference.checked = reference.dataset.beforeSkipChecked !== '0';
            delete reference.dataset.beforeSkipChecked;
          }
          reference.disabled = skipped;
          form.querySelector('[data-simple-application="' + i + '"]').textContent = skipped ? skipMessage : '';
          ...
          form.querySelectorAll('[data-target-entry="' + i + '"],[data-otsu-entry="' + i + '"]').forEach(el => {
            const excludedKey = excluded.has(el.dataset.targetKey || el.dataset.otsuKey);
            el.disabled = excludedKey || (skipped && el.hasAttribute('data-target-entry'));
            if (excludedKey) el.checked = false;
          });
          const selected = skipped ? [] : [...form.querySelectorAll('[data-target-entry="' + i + '"]:checked')].map(el => el.dataset.targetKey);
          total += selected.length;
          form.querySelector('[data-target-summary="' + i + '"]').textContent = skipped ? skipMessage : numeric(entry).filter(m => selected.includes(m.key)).map(m=>m.name || m.key).join('、') || '選択なし';
        });
        form.querySelector('#normalization-targets > summary').textContent = '補正対象の分子を確認（合計 ' + total + ' レイヤー）';
        form.querySelector('#normalization-simple-summary').textContent = '補正予定 ' + (entries.length - skippedCount) + '件 ／ 内部標準なしでスキップ ' + skippedCount + '件' +
          (skippedCount ? '。スキップしたデータは未補正として記録し、生値で表示します。' : '');
      }
      updateSelections();
      ...
      function configFromForm() {
        const activeCount = entries.filter((entry,i) => !isSkippedEntry(i)).length;
        ...
        entries.forEach((entry,i) => {
          ...
          const skipped = isSkippedEntry(i);
          if (!skipped && (!standardKey || standardKey === '__unresolved__')) throw new Error((entry.project.displayName || entry.project.id) + '：内部標準の d4-5-HT を指定してください。');
          if (!skipped && htKey === '__unresolved__') throw new Error((entry.project.displayName || entry.project.id) + '：5-HT の候補を確認して選択してください。');
          entry.simpleMapping = {standardKey:standardKey || null,htKey:skipped ? null : htKey || null,targetKeys:skipped ? [] : [...form.querySelectorAll('[data-target-entry="' + i + '"]:checked')].map(el=>el.dataset.targetKey)};
          if (!skipped && form.elements['simple_ref_' + i].checked) config.reference.projectIds.push(entry.project.id);
          config.otsuSourceKeys[entry.project.id] = [...form.querySelectorAll('[data-otsu-entry="' + i + '"]:checked')].map(el=>el.dataset.otsuKey);
        });
        if (activeCount && !config.reference.projectIds.length) throw new Error('固定基準データを少なくとも 1 件選択してください。');
        if (activeCount && config.reference.kind === 'roi' && !config.reference.roiNames.length) throw new Error('固定基準 ROI 名を指定してください。');
        if (!activeCount && form.elements.calibrationEnabled.checked) config.calibration = seed.calibration || null;
        if (activeCount && form.elements.calibrationEnabled.checked) {
      ...
      async function calculate() {
        ...
        const skippedCount = preview.preview.filter(row=>row.skipped).length;
        result.innerHTML = '<p>対象 ' + entries.length + '件 ／ 係数算出可能 ' + preview.preview.filter(row=>!row.skipped && Number.isFinite(row.k)).length + '件' +
          (skippedCount ? ' ／ 内部標準なしでスキップ ' + skippedCount + '件' : '') + '</p>' +
          ...
          preview.preview.map(row=>'<tr data-simple-output="' + esc(row.projectId) + '"><td style="padding:8px;border-top:1px solid #cbd5e1">' + esc(row.name || row.projectId) + '</td><td>' + fmt(row.Ds) + '</td><td>' + fmt(row.Dref) + '</td><td>' + fmt(row.k) +
            '</td><td>' + (row.skipped ? '<div style="color:#92400e">' + skipMessage + '</div>' : (row.correctedOutputs || []).map(ch=>'<div>' + esc(ch.name || ch.key) + '：' + ch.nValid + ' / ' + ch.nMeasured + '画素' + (ch.nValid < ch.nMeasured ? '（一部画素から算出）' : '') + (ch.nValid ? '' : ' ／ ' + esc(reason(ch.reasonCodes))) + '</div>').join('')) +
            (!row.skipped && !Number.isFinite(row.k) ? '<div style="color:#b91c1c">' + esc(reason(row.reasonCodes)) + '</div>' : '') + '</td></tr>').join('') + '</tbody></table>' +
          (skippedCount ? '<p style="font-size:12px">スキップしたデータは未補正として記録し、生値で表示します。基準値・共通色レンジの計算には含めません。</p>' : '') +
          '<p style="font-size:12px">生値は保持します。' + (skippedCount === entries.length ? '全件が内部標準なしのため、補正値・係数を算出せず、未補正として記録します。' :
            (config.qc.enforceCoverage ? '基準領域・ROI の最低有効率を適用します。' : '有効画素がある ROI は、その画素から平均・SDを算出し、有効率を併記します。') + (config.qc.saturationD4 == null ? ' 飽和未確認の相対補正です。' : '')) + '</p>' +
          (!preview.canSave ? '<p id="normalization-simple-blocked" style="color:#b91c1c">' + (skippedCount ? '補正対象の係数と補正値を確認できないため保存できません。内部標準の未登録以外の問題をスキップすることはありません。' : '全件の係数と補正値を確認できないため保存できません。対象を自動で減らすことはありません。') + esc(reason(preview.reasonCodes)) + '</p>' : '');
        return preview;
      }
      function savedCompletion() {
        const skippedCount = saved.filter(p=>N.isSkippedProfile(p.normalization)).length;
        return saved.length + ' 件をこの PC に保存しました。' + (skippedCount ? ' 補正 ' + (saved.length - skippedCount) + '件 ／ 内部標準なしでスキップ ' + skippedCount + '件。スキップしたデータは未補正として記録し、生値で表示します。' : '');
      }
      async function syncSaved() {
        if (!saved || !options.saveCloud || !cloudPending.size || savedKind !== 'normalization') return;
        const failures = [];
        ...
        status.textContent = savedCompletion() + 'クラウド同期 ' + (saved.length - cloudPending.size) + '/' + saved.length + ' 件。' +
          (failures.length ? '\n未完了分は再試行できます。\n' + failures.join('\n') : '\nすべて同じプロファイル・revision の保存を確認しました。');
        status.style.color = failures.length ? '#92400e' : '#1e293b';
        if (options.onSaved) {
          try { await options.onSaved(saved); }
      ...
      form.onsubmit = async event => {
        ...
        try {
          ...
          status.textContent = savedCompletion();
          ...
          updateDisplayControls();
        } catch (e) { setBusy(false); error(e); form.querySelector('#normalization-simple-cloud').disabled = savedKind !== 'normalization' || !cloudPending.size; }
      };
      form.querySelector('#normalization-simple-cloud').onclick = async () => {
        if (busy || !saved || !cloudPending.size || savedKind !== 'normalization') return;
