...
(function (global) {
  ...
  /** Create only. A colliding project ID must never replace an existing row. */
  async function insertRowIfAbsent(row) {
    const body = Object.assign({}, row, { updated_at: new Date().toISOString(), updated_by: who() });
    const res = await call('/rest/v1/' + TABLE, {
      method: 'POST',
    ...
    const rows = await res.json();
    if (!rows || !rows.length) throw CloudError('クラウドへの新規登録を確認できませんでした', 409, '');
    return rows[0];
  }
  ...
  async function patchRowIfUnchanged(id, patch, expectedUpdatedAt) {
    if (!expectedUpdatedAt) throw CloudError('保存元のクラウド版を確認できません。最新の設定を読み込んでください', 409, '');
    // A same-millisecond write must still advance the CAS token. Otherwise a
    // second stale writer could match the unchanged timestamp and overwrite it.
    const updatedAt = new Date(Math.max(Date.now(), (Date.parse(expectedUpdatedAt) || 0) + 1)).toISOString();
    const body = Object.assign({}, patch, { updated_at: updatedAt, updated_by: who() });
  ...
  /** Delete only the exact source version reviewed before a merge/removal. */
  async function removeRowIfUnchanged(id, expectedUpdatedAt) {
    if (!expectedUpdatedAt) throw CloudError('削除元のクラウド版を確認できません。最新の設定を読み込んでください', 409, '');
    const q = '/rest/v1/' + TABLE + '?id=eq.' + encodeURIComponent(id) +
      '&updated_at=eq.' + encodeURIComponent(expectedUpdatedAt);
    const res = await call(q, { method: 'DELETE', headers: { 'Prefer': 'return=representation' } });
    const rows = await res.json();
  ...
  const STATE_KEYS = ['roi', 'alignment', 'layerDisplay', 'otsu', 'visibleLayers',
                      'world_coords', 'rotation', 'normalization', 'normalizationBinding', 'valueDisplay', 'stack3d'];
  ...
  /** An authoritative row replaces the shared state, including absent keys. */
  function replaceState(project, state) {
    for (const k of STATE_KEYS) delete project[k];
    return applyState(project, state || {});
  }
  ...
  function metaOf(project) {
    ...
    return {
      ...
      hasHe: !!(project.images && project.images.HE_Stain),
      hasAtlas: !!(project.images && project.images[global.ATLAS_KEY || 'ATLAS']),
      hasImmuno: !!(project.images && project.images[global.IMMUNO_KEY || 'IMMUNO']),
      ...
      normalization: profile ? {
        ...
        ...(profile.schemaVersion === 3 ? {
          ...
          standardKey: profile.mapping && profile.mapping.d4 || null,
          ...(profile.application ? { application: { status: profile.application.status, reasonCode: profile.application.reasonCode } } : {}),
          targets: (Array.isArray(profile.targets) ? profile.targets : []).filter(Boolean).map(target => ({ key: target.key, method: target.method,
            analyteId: target.analyteId, rangeScope: target.rangeScope })),
        } : {}),
        groupId: profile.scope && profile.scope.groupId || null,
        folderPath: profile.scope && profile.scope.folderPath || null,
        memberCount: profile.scope && Array.isArray(profile.scope.memberIds) ? profile.scope.memberIds.length : null,
        batchId: profile.batchId || '', prepId: profile.prepId || '',
        quality: profile.quality || '',
        factor: profile.section && Number.isFinite(profile.section.k) ? profile.section.k : null,
        // This is a saved-profile summary, not evidence that current raw data pass QC.
        status: profile.scope && project.normalizationBinding && project.normalizationBinding.groupId !== profile.scope.groupId
          ? 'GROUP_MEMBERSHIP_CHANGED' : profile.invalidated ? 'STALE' :
            profile.schemaVersion === 3 && profile.application && profile.application.status === 'skipped' &&
            profile.application.reasonCode === 'INTERNAL_STANDARD_MISSING' ? 'SKIPPED' : 'SAVED_NOT_REEVALUATED',
      } : null,
  ...
  global.Cloud = {
    ...
    putRow: putRow,
    insertRowIfAbsent: insertRowIfAbsent,
    patchRow: patchRow,
    patchRowIfUnchanged: patchRowIfUnchanged,
    removeRow: removeRow,
    removeRowIfUnchanged: removeRowIfUnchanged,
