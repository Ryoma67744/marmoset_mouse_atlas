...
(function (global) {
  ...
  function assess(project, group) {
    ...
    if (binding && binding.groupId !== scope.groupId) return { status: 'MOVED', reason: '補正時と現在のグループが異なります。現在のグループで再設定してください。', numericalSnapshotPreserved: true };
    ...
    if (!group) return { status: 'CURRENT', reason: '保存済みの基準値・係数を使用します。現在のグループ全体の構成は未確認です。', membershipVerified: false };
    ...
    const scoped = (group.projects || []).filter(p => p.normalization && p.normalization.scope), signatures = new Set(scoped.map(p => JSON.stringify([p.normalization.scope.groupId, p.normalization.id, p.normalization.revision, p.normalization.scope.memberIds])));
  ...
  global.NormalizationScope = { buildGroups: buildGroups, groupForFolder: groupForFolder, memberId: memberId, snapshot: snapshot, equalsSnapshot: equalsSnapshot, assess: assess };
})(window);
