...
(function (global) {
  ...
  // Export
  // =====================================================================
  async function exportProject(project, deps, options) {
    if (typeof JSZip === 'undefined') throw new Error('JSZip が読み込まれていません');
    const storage = (deps && deps.storage) || global.ProjectStorage;
    // 一括バックアップでは欠けたデータを成功扱いにしない。
    // 個別 Export / クラウド保存は従来の許容動作を維持する。
    const strict = !!(options && options.strict === true);
    const zip = new JSZip();
    const base = safeName(project.displayName || project.id);

    // ---- 生値 CSV -------------------------------------------------------
    const table = await prepareProjectTable(project, { storage: storage }, { strict: strict });
    ...
    for (const key of Object.keys(project.images || {})) {
      ...
      if (!im || !im.blobId) {
        if (strict) throw new Error('登録済み画像 ' + key + ' の保存先が見つからないため、復元用 ZIP を書き出せません');
        continue;
      }
      const rec = await storage.getBlob(im.blobId);
      if (!rec || !rec.blob || (strict && rec.blob.size === 0)) {
        if (strict) throw new Error('登録済み画像 ' + (im.filename || key) + ' の原本が見つからないか空のため、復元用 ZIP を書き出せません');
        continue;
      }
      ...
      const dir = key === (global.ATLAS_KEY || 'ATLAS') ? 'Atlas/'
                ...
                : 'HE/';
      const stem = safeName(fname.replace(/\.[^.]+$/, ''));
      const extension = safeName((/\.[^.]+$/.exec(fname) || ['.png'])[0]);
      let path = dir + stem + extension;
      let suffix = 2;
      // 同じ原本名や safeName 後に同名となる画像も、それぞれを保持する。
      // 大文字小文字を区別しない展開先でも上書きされないようにする。
      while (imagePaths.has(path.toLowerCase())) path = dir + stem + '__' + suffix++ + extension;
      imagePaths.add(path.toLowerCase());
    ...
    const manifest = {
      ...
      // Optional placement only; old projects must retain their original cloud hash.
      stack3d: project.stack3d,
  ...
  async function importNative(zip, doc, storage, forceId) {
    ...
    if (Object.prototype.hasOwnProperty.call(doc, 'stack3d')) project.stack3d = doc.stack3d;
