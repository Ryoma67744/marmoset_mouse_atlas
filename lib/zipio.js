...
(function (global) {
  ...
  async function importZip(file, deps) {
    ...
    if (found.kind === FORMAT) {
      ...
      // A cloud bundle freezes raw data, while its separate row has the latest
      // folder and state. Apply that authoritative metadata before resolving
      // folders or restoring UUIDs; otherwise an obsolete bundle can recreate a
      // moved folder or conflict with an unrelated group at the old path.
      if (deps && Object.prototype.hasOwnProperty.call(deps, 'state')) {
        if (!global.Cloud || typeof global.Cloud.applyState !== 'function') {
          throw new Error('クラウドの最新設定を復元できません。アプリを再読み込みしてください');
        }
        global.Cloud.applyState(doc, deps.state);
      }
      if (deps && Object.prototype.hasOwnProperty.call(deps, 'folderPath')) doc.folderPath = deps.folderPath;
      if (deps && deps.displayName) doc.displayName = deps.displayName;
      return importNative(zip, doc, storage, deps && deps.id);
    }
  ...
  async function importNative(zip, doc, storage, forceId) {
    ...
    // The immutable profile retains its original membership and references.
    // Only the separate current folder identity is restored, and only if vacant.
    // A conflict must stop before rasters/projects are written, never replace an
    // existing folder's UUID because another ZIP happens to use the same names.
    if (doc.normalizationBinding && doc.normalizationBinding.groupId && (doc.folderPath || []).length >= 2) {
      if (typeof storage.restoreNormalizationGroup !== 'function') {
        throw new Error('補正グループを安全に復元できません。アプリを再読み込みしてください');
      }
      await storage.restoreNormalizationGroup(folderId, doc.normalizationBinding.groupId);
    }
    // 同じ id で上書きするときは、古いほうが持っていた blob を控えておく。
    // putProject は id 上書きなので、ここで拾わないと生値ラスタと HE が
    // 参照されないまま IndexedDB に残り続ける (取り込むたびに増える)。
    const prev = forceId ? await storage.getProject(forceId) : null;
    const project = {
    ...
    // 上書きが済んでから古い blob を片付ける (先に消すと、途中で失敗したときに
    // 元のデータまで壊れる)。
    if (prev) {
      for (const bid of storage.collectBlobIds(prev)) {
        try { await storage.deleteBlob(bid); } catch (e) { console.warn('[zipio] 古い blob の削除に失敗', bid, e); }
      }
    }
