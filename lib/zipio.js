...
(function (global) {
  ...
  async function exportProject(project, deps) {
    ...
    // ★ 先頭のレコードだけを見ない。1 つの ROI は離れた複数の場所を持てるので、
    //   1 箇所目が壊れていても 2 箇所目が使えるなら列として出す。
    const roiKeys = Object.keys(roiItems).filter(k => {
      const recs = roiItems[k];
      return Array.isArray(recs) && recs.some(r => {
        const p = r && r.poly_msi;
  ...
  async function importZip(file, deps) {
    ...
    if (found.kind === FORMAT) return importNative(zip, found.doc, storage, deps && deps.id);
