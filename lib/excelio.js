...
(function (global) {
  ...
  function groupProvenance(project, context) {
    const profile = project.normalization || {}, scope = profile.scope;
    const binding = project.normalizationBinding;
    const explicitBinding = Object.prototype.hasOwnProperty.call(project, 'normalizationBinding');
    return {
      groupId: scope && scope.groupId || '',
      scopePath: folderPathValue(scope && scope.folderPath),
      currentPath: folderPathValue(context && context.hierarchyAvailable ? context.group && context.group.path || [] : explicitBinding ? binding && binding.folderPath
        : Array.isArray(project.folderPath) && project.folderPath.length >= 2 ? project.folderPath.slice(0, 2) : ''),
      groupStatus: context && context.hierarchyAvailable ? context.assessment.status : !scope ? project.normalization ? 'LEGACY' : 'UNCONFIGURED' : explicitBinding
        ? binding && binding.groupId === scope.groupId ? 'SAVED_GROUP' : 'MOVED' : 'SAVED_SNAPSHOT',
  ...
  function isCorrectedChannel(channel) {
    if (global.Normalization && typeof global.Normalization.isCorrectedChannel === 'function') {
      return global.Normalization.isCorrectedChannel(channel);
    }
    return !!channel && ['pixel_ratio', 'section_scale', 'section_factor'].includes(channel.method);
  }

  function channelRange(profile, channel) {
    if (global.Normalization && typeof global.Normalization.rangeForChannel === 'function') {
      return global.Normalization.rangeForChannel(profile, channel);
    }
    return profile && profile.commonRanges && profile.commonRanges[channel && channel.role] || null;
  }

  function isSkippedProfile(profile) {
    return !!(global.Normalization && typeof global.Normalization.isSkippedProfile === 'function' &&
      global.Normalization.isSkippedProfile(profile));
  }

  // DisplayRange may migrate old settings and bind cached windows to the current
  // data. Export uses detached display state so building a workbook never saves
  // a migration, changes a scientific profile or alters the open Viewer.
  function prepareDisplayProvenance(project, table, analysis) {
    const displayProject = Object.assign({}, project, {
      valueDisplay: JSON.parse(JSON.stringify(project.valueDisplay || {})),
      layerDisplay: JSON.parse(JSON.stringify(project.layerDisplay || {})),
    });
    const out = { project: displayProject, channels: [], error: '' };
    const engine = global.DisplayRange;
    if (!engine || typeof engine.resolve !== 'function' || typeof engine.prepareProject !== 'function') {
      out.error = 'DISPLAY_RANGE_ENGINE_UNAVAILABLE';
      return out;
    }
    try {
      engine.prepareProject(displayProject);
      const skipped = isSkippedProfile(project.normalization);
      // Mirror Viewer initialization. prepareProject's raw default must not
      // override a valid profile's inferred normalized mode when none was saved.
      const savedMode = project.valueDisplay && project.valueDisplay.mode;
      const activeMode = !project.normalization || skipped ? 'raw'
        : ['normalized', 'raw'].includes(savedMode) ? savedMode
          : ['VALID', 'PROVISIONAL', 'PARTIAL'].includes(analysis.evaluation.status) ? 'normalized' : 'raw';
      displayProject.valueDisplay.mode = activeMode;
      table.molecules.forEach(m => {
        const channel = analysis.evaluation.channels[m.key] || {};
        for (const requestedMode of ['raw', 'normalized']) {
          const mode = requestedMode === 'normalized' && project.normalization && !skipped && isCorrectedChannel(channel)
            ? 'normalized' : 'raw';
          const result = engine.resolve({ project: displayProject, key: m.key, mode: mode,
            values: mode === 'normalized' ? channel.values : m.raster,
            channel: channel, rawFingerprint: analysis.evaluation.fingerprint || '' });
          out.channels.push({ key: m.key, name: m.name, requestedMode: requestedMode, mode: mode,
            active: requestedMode === activeMode, unit: mode === 'normalized' ? channel.unit || '' : 'raw a.u.',
            range: result });
        }
      });
    } catch (error) {
      // Display provenance failure must not suppress original or derived data.
  ...
  function normalizedHeaders(table, analysis) {
    const skipped = isSkippedProfile(analysis.evaluation.profile);
    const values = table.molecules.map(m => {
      // The original registered name is preserved, even for unsupported channels.
      const channel = analysis.evaluation.channels[m.key] || {};
      const label = skipped ? 'not normalized: internal standard absent; see Data'
        : isCorrectedChannel(channel) && channel.method === 'pixel_ratio' ? '5-HT / d4-5-HT ratio'
        : isCorrectedChannel(channel) ? 'section normalized intensity'
  ...
  function normalizedRow(sourceRow, table, analysis) {
    ...
    table.molecules.forEach(m => {
      const channel = analysis.evaluation.channels[m.key] || {};
      const corrected = isCorrectedChannel(channel);
      const value = corrected && channel.values ? finiteCell(channel.values[index]) : null;
  ...
  function roiQuantificationRows(project, table, analysis, options) {
    const profile = project.normalization || {}, provenance = groupProvenance(project, analysis.context);
    const rows = [[
      ...
      'Analyte ID', 'Display range key', 'Display range scope', 'Fixed display minimum', 'Fixed display maximum',
    ...
    roiDefinitionsForQuantification(project, table).forEach(roi => {
      ...
      results.forEach(result => {
        ...
        const range = channelRange(profile, channel);
        ...
        rows.push([roi.key, roi.name, result.key, result.name, result.role,
          ...
          profile.revision == null ? '' : profile.revision, finiteCell(profile.section && profile.section.k),
          channel.analyteId || '', channel.rangeKey || '', channel.rangeScope || (range ? 'group' : ''),
          finiteCell(range && range.min), finiteCell(range && range.max), visible]);
  ...
  function normalizationSummary(project, analysis, roiResults) {
    ...
    const skipped = isSkippedProfile(profile);
    const absoluteApplicable = !skipped && (Number(profile.schemaVersion) !== 3 ||
      (Array.isArray(profile.targets) && profile.targets.some(target => target.method === 'pixel_ratio')));
    const absolute = absoluteApplicable ? roiResults.summaries.filter(r => r.role === 'ht').map(r => r.absolute || {}) : [];
    const absCodes = [...new Set(absolute.flatMap(r => codes(r.reasonCodes)))];
    if (absoluteApplicable && !absolute.length) {
      absCodes.push('NO_ROI_QUANTIFICATION');
      if (evaluation.profile && !profile.calibration) absCodes.push('CALIBRATION_MISSING');
    }
    return {
      ...groupProvenance(project, analysis.context),
      ...
      skipped: skipped, skipReason: skipped ? profile.application.reasonCode : '',
      absoluteStatus: !absoluteApplicable ? 'NOT_APPLIED' : absolute.length ? [...new Set(absolute.map(a => a.status || 'UNAVAILABLE'))].join(' | ') : 'UNAVAILABLE',
  ...
  function normalizationMetadataRows(project, table, analysis, summary, headers) {
    const rows = [
      ['Field', 'Value'], ['Format', Number(summary.schemaVersion) >= 3 ? 'marmoset_atlas_normalization_v3' : 'marmoset_atlas_normalization_v2'],
      ['Status', summary.status], ['Reason codes', summary.reasonCodes.join(' | ')], ['Reason', summary.reasonText],
      ['Correction skipped', summary.skipped], ['Correction skip reason', summary.skipReason],
      ['Skipped data convention', 'When the d4-5-HT internal standard is absent, correction is skipped. Data and Raw ROI columns retain original values; Normalized_Data and normalized ROI values remain blank. Raw values are not corrected values.'],
      ...
      ['Group comparison', summary.skipped ? 'No correction was applied to this dataset. It does not contribute to the group reference or corrected display ranges. Raw values are not on the corrected group comparison scale.' : summary.groupId ? 'Section-scale analytes use an independently fixed reference within this normalization group. Separate groups are not a shared comparison scale.' : 'Legacy profile: no folder-group scope was recorded.'],
      ['Group membership status convention', 'CURRENT: current hierarchy agrees with the saved group; see verification fields. COMPOSITION_CHANGED / MIXED: saved coefficients are retained and the group needs review. MOVED: current group differs and derived values are unavailable. SAVED_GROUP / SAVED_SNAPSHOT: only saved membership is available. LEGACY: no saved folder scope. UNCONFIGURED: no profile.'],
      ['Current folder hierarchy verified', !!(analysis.context && analysis.context.hierarchyAvailable)],
      ['Group membership verified', !!(analysis.context && analysis.context.assessment.membershipVerified)],
      ['Group assessment', analysis.context && analysis.context.assessment.reason || 'Current group context was not available.'],
      ...
      ['Other selected analytes formula', 'Schema 3 section_scale targets: raw intensity * fixed section.k (Dref / Ds). Local d4 need not be measurable at every target pixel. The shared d4 reference provides relative correction, not analyte-specific validation or absolute quantification.'],
      ['D4 and excluded channels', 'The selected d4 internal standard is raw QC only; excluded channels have no derived value. Consult Data for their raw intensities.'],
      ['ROI valid coverage', 'Normalized n / measured raw n within the analytical ROI. Schema 3 simple profiles report partial coverage without suppressing finite means unless qc.enforceCoverage is true. Legacy profile coverage rules remain unchanged.'],
      ['Display range convention', 'Display limits do not clip exported analytical values. Fixed ranges are separate per analyte; schema 3 duplicate or unknown names use an individual range. An individual range does not claim a shared group color scale.'],
      ['Effective display convention', 'The original Fixed display columns retain the immutable calculation-profile ranges for compatibility. Effective display limits below are resolved separately from current display settings. P99 clips brightness only; all finite exported analytical values are retained. Individual windows do not share a color scale between sections.'],
      ['Display provenance error', analysis.display.error],
    ...
    appendSnapshot('layerDisplay', project.layerDisplay || {});
    appendSnapshot('resolvedDisplay.valueDisplay', analysis.display.project.valueDisplay || null);
    appendSnapshot('resolvedDisplay.layerDisplay', analysis.display.project.layerDisplay || {});
    ...
    table.molecules.forEach((m, index) => {
      ...
      const range = channelRange(project.normalization, c);
      rows.push([m.key, m.name, c.role || '', c.method || '', c.unit || '', headers[2 + index],
        ...
        c.analyteId || '', c.rangeKey || '', c.rangeScope || (range ? 'group' : ''),
        finiteCell(range && range.min), finiteCell(range && range.max)]);
    });
    rows.push([], ['Effective display molecule key', 'Molecule name', 'Requested value mode', 'Displayed value mode',
      'Active value mode', 'Displayed unit', 'Requested strategy', 'Effective strategy', 'Effective display minimum',
      'Effective display maximum', 'Actual minimum', 'Actual maximum', 'Display quantile', 'Finite pixels',
      'Clipped pixels', 'Clipped fraction', 'Display source', 'Display status', 'Display reason',
      'Display group ID', 'Display snapshot ID', 'Display identity', 'Display group members',
      'Display group contributors', 'Display group finite pixels', 'Display group actual minimum',
      'Display group actual maximum', 'Display group clipped fraction', 'Display group maximum source']);
    analysis.display.channels.forEach(item => {
      const r = item.range || {};
      rows.push([item.key, item.name, item.requestedMode, item.mode, item.active, item.unit,
        r.requestedStrategy || '', r.strategy || '', finiteCell(r.min), finiteCell(r.max),
        finiteCell(r.actualMin), finiteCell(r.actualMax), finiteCell(r.quantile), finiteCell(r.nFinite),
        finiteCell(r.nClipped), finiteCell(r.clippedFraction), r.source || '', r.status || '', r.reason || '',
        r.groupId || '', r.snapshotId || '', typeof r.identity === 'string' ? r.identity : safeJSON(r.identity),
        finiteCell(r.groupMemberCount), finiteCell(r.groupContributorCount), finiteCell(r.groupNFinite),
        finiteCell(r.groupActualMin), finiteCell(r.groupActualMax), finiteCell(r.groupClippedFraction),
        r.maximum ? safeJSON(r.maximum) : '']);
    });
    return rows;
  }
  ...
  async function buildProjectXlsx(project, deps, options) {
    ...
    let context = null;
    if (global.NormalizationScope && typeof global.NormalizationScope.resolveContext === 'function') {
      const folders = storage && typeof storage.listFolders === 'function' ? await storage.listFolders() : undefined;
      const projects = Array.isArray(folders) && storage && typeof storage.listProjects === 'function' ? await storage.listProjects() : undefined;
      context = global.NormalizationScope.resolveContext(project, folders, projects);
    ...
    analysis.display = prepareDisplayProvenance(project, table, analysis);
