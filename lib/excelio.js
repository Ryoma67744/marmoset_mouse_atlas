...
(function (global) {
  ...
  function normalizedHeaders(table, analysis) {
    const values = table.molecules.map(m => {
      ...
      const label = isCorrectedChannel(channel) && channel.method === 'pixel_ratio' ? '5-HT / d4-5-HT ratio'
  ...
  function normalizationSummary(project, analysis, roiResults) {
    ...
    const absoluteApplicable = Number(profile.schemaVersion) !== 3 ||
  ...
  function normalizationMetadataRows(project, table, analysis, summary, headers) {
    const rows = [
      ...
      ['Group comparison', summary.groupId ? 'Section-scale analytes use an independently fixed reference within this normalization group. Separate groups are not a shared comparison scale.' : 'Legacy profile: no folder-group scope was recorded.'],
