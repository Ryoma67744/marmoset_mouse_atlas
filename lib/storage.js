...
(function (global) {
  ...
  const ProjectStorage = {
    ...
    async patchProjectFields(id, fields, options) {
      ...
      // Derived edits may depend on neighboring projects. Snapshot the caller's
      // revision list before yielding, then validate it under the same write
      // lock as the selected project. Preflight reads alone leave a race window.
      let expectedProjectRevisions = [];
      if (options && Object.prototype.hasOwnProperty.call(options, 'expectedProjectRevisions')) {
        const source = options.expectedProjectRevisions;
        if (!Array.isArray(source) || Array.from(source).some(item => !item || typeof item !== 'object' || Array.isArray(item) ||
            typeof item.id !== 'string' || !item.id.trim() || typeof item.updatedAt !== 'string' || !item.updatedAt.trim())) {
          throw new Error('参照データのIDと更新日時の一覧が不正です');
        }
        expectedProjectRevisions = source.map(item => ({ id: item.id, updatedAt: item.updatedAt }));
        if (new Set(expectedProjectRevisions.map(item => item.id)).size !== expectedProjectRevisions.length) {
          throw new Error('参照データのIDが重複しています');
        }
      }
      ...
      return new Promise((resolve, reject) => {
        ...
        const ids = Array.from(new Set([id, ...expectedProjectRevisions.map(item => item.id)])), records = new Map();
        let remaining = ids.length;
        for (const projectId of ids) {
          const req = s.get(projectId);
          req.onsuccess = () => {
            records.set(projectId, req.result);
            if (--remaining) return;
            try {
              const old = records.get(id);
              if (!old) throw conflictError('対象データが削除されています');
              if (options && Object.prototype.hasOwnProperty.call(options, 'expectedUpdatedAt')) checkRevision(old, options.expectedUpdatedAt);
              for (const reference of expectedProjectRevisions) {
                try { checkRevision(records.get(reference.id), reference.updatedAt); }
                catch (_) { throw conflictError('参照データ（' + reference.id + '）が更新または削除されています。最新の内容で候補を再計算してください'); }
              }
    ...
    /** Save a whole-stack placement proposal atomically, pinning every reference used to derive it. */
    async saveStack3DPlacements(placements, options) {
      const owns = (value, key) => Object.prototype.hasOwnProperty.call(value, key);
      const record = value => value && typeof value === 'object' && !Array.isArray(value);
      const validId = value => typeof value === 'string' && !!value.trim();
      // Copy only placement/revision primitives before opening the database. The
      // caller can continue editing a proposal without changing this transaction.
      if (!Array.isArray(placements) || !placements.length || Array.from(placements).some(item => !record(item) ||
          !owns(item, 'id') || !validId(item.id) || ['offsetXUm', 'offsetYUm', 'rotationDeg'].some(key =>
            !owns(item, key) || typeof item[key] !== 'number' || !Number.isFinite(item[key])))) {
        throw new Error('一括保存する切片のIDと有限の配置値が必要です');
      }
      const targets = Array.from(placements, item => ({ id: item.id, offsetXUm: item.offsetXUm,
        offsetYUm: item.offsetYUm, rotationDeg: item.rotationDeg }));
      if (new Set(targets.map(item => item.id)).size !== targets.length) throw new Error('保存対象の切片IDが重複しています');
      const source = record(options) && owns(options, 'expectedProjectRevisions') && options.expectedProjectRevisions;
      if (!Array.isArray(source) || !source.length || Array.from(source).some(item => !record(item) ||
          !owns(item, 'id') || !validId(item.id) || !owns(item, 'updatedAt') || !validId(item.updatedAt))) {
        throw new Error('参照データのIDと更新日時の一覧が必要です');
      }
      const references = Array.from(source, item => ({ id: item.id, updatedAt: item.updatedAt }));
      const referenceIds = new Set(references.map(item => item.id));
      if (referenceIds.size !== references.length) throw new Error('参照データのIDが重複しています');
      if (targets.some(item => !referenceIds.has(item.id))) throw new Error('すべての保存対象の更新確認が必要です');
      const db = await openDb();
      return new Promise((resolve, reject) => {
        const t = db.transaction(STORE_PROJECTS, 'readwrite'), s = t.objectStore(STORE_PROJECTS);
        const records = new Map();
        let remaining = references.length, result, failure;
        t.oncomplete = () => { notifyChange(targets.map(item => item.id)); resolve(result); };
        t.onerror = t.onabort = () => reject(failure || t.error || new Error('配置の一括保存に失敗しました'));
        for (const reference of references) {
          const req = s.get(reference.id);
          req.onsuccess = () => {
            records.set(reference.id, req.result);
            if (--remaining) return;
            try {
              // Read and check all targets and neighboring references under the
              // same write lock before issuing any placement writes.
              for (const expected of references) {
                try { checkRevision(records.get(expected.id), expected.updatedAt); }
                catch (_) { throw conflictError('参照データ（' + expected.id + '）が更新または削除されています。最新の内容で候補を再計算してください'); }
              }
              result = targets.map(item => {
                const old = records.get(item.id);
                return Object.assign({}, old, { stack3d: { schemaVersion: 1, offsetXUm: item.offsetXUm,
                  offsetYUm: item.offsetYUm, rotationDeg: item.rotationDeg }, updatedAt: revisionAfter(old) });
              });
              for (const project of result) s.put(project);
            } catch (error) { failure = error; t.abort(); }
          };
        }
      });
