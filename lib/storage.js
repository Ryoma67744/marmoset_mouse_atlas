...
(function (global) {
  ...
  const ProjectStorage = {
    ...
    async putProjectsIfUnchanged(updates) {
      ...
      return new Promise((resolve, reject) => {
        ...
        for (const item of updates) {
          const req = s.get(item.project.id);
          req.onsuccess = () => {
            const old = req.result;
            if (!old || old.updatedAt !== item.expectedUpdatedAt) {
              failure = new Error('プレビュー後にデータが変更されました。再読み込みして設定をやり直してください: ' + item.project.displayName);
              t.abort();
              return;
            }
        ...
        t.oncomplete = () => resolve(updates.map(item => item.project));
  ...
  async function ensureFolderPath(names) {
    // 一覧は最初に 1 回だけ取り、作ったぶんは手元の配列にも足していく。
    // ★ ループの中で listFolders() を呼び直すと、複数の取り込みが同時に走ったとき
    //   互いの作りたてを見落として同じ名前のフォルダが並ぶ。
