...
(function (global) {
  ...
  global.Stack3DRoi = { create(T, doc, section) {
    const group = new T.Group(), records = [];
    ...
    for (const polygon of global.Stack3D.roiPolygons(section).filter(p => p.visible)) {
      ...
      const color = new T.Color(...polygon.color.map(v => v / 255));
      const line = new T.LineLoop(geometry, new T.LineBasicMaterial({ color, transparent: true, opacity: 0.85, depthWrite: false, depthTest: false }));
    ...
    for (const name of new Set(records.map(r => r.name))) {
      ...
      const sprite = new T.Sprite(new T.SpriteMaterial({ map: texture, transparent: true, depthWrite: false, depthTest: false }));
    ...
    return { group, update({ visible, region, labelsVisible }) {
      ...
      labels.forEach(r => { r.texture.dispose(); r.sprite.material.dispose(); }); group.clear();
    } };
  } };
})(window);
